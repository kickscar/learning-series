...
5. **문서 반영 여부**
   - **기본:** 대화로만 제안한다.
   - 사용자가 **「Rev에 올려줘」「문서에 반영」**했거나, Handoff에서 문서 갱신이 합의된 경우에만 **`gallery-spring-boot.md` 등에 새 `## Rev N` 과 체크리스트를 append**하고 **Revision 이력 표**를 갱신한다.
...
- 예외: 사용자가 **「문서에 반영」**만 요청한 경우 → 해당 `.md` 만 수정 가능.
