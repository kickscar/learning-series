# 6.2 aiohttp Async HTTP Server

## 개요

본 Section은 **aiohttp**를 사용한 비동기 HTTP 서버 구동을 다룬다. 6.1의 ThreadingHTTPServer는 요청마다 스레드를 생성하는 반면, aiohttp는 **이벤트 루프**와 **논블로킹 IO**로 단일 스레드에서 동시에 여러 요청을 처리한다. 학습 흐름은 다음과 같다. ① 6.1과의 다중 요청 처리 차이를 이해한다. ② aiohttp로 비동기 HTTP 서버를 구동한다.

## 이론

### 6.1 http.server와의 차이

| 구분 | 6.1 ThreadingHTTPServer | 6.2 aiohttp |
|------|--------------------------|-------------|
...
- aiohttp는 이벤트 루프 기반 비동기 HTTP 서버로, 6.1의 스레드 모델과 다르다.
