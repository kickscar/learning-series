...
본 Section은 **Event loop**와 **Async IO**의 개념을 다룬다. Ch02에서 selectors, Thread-per-request로 다중 요청을 처리했으나, 비동기 모델은 스레드·프로세스 없이 단일 스레드에서 많은 동시 연결을 처리할 수 있다. 학습 흐름은 다음과 같다. ① Event loop와 Async IO의 동작 방식을 이해한다. ② `asyncio.run()`, `asyncio.gather()`의 역할과 Python 비동기 원리(코루틴·Task·Cooperative multitasking)를 파악한다. ③ Ch02 대비 성능·리소스 이점을 파악한다. 실습은 4.2·4.3 asyncio에서 진행한다.
...
IO-bound 작업이 많을 때 비동기는 스레드보다 메모리와 컨텍스트 스위칭 비용이 적다.

## 실습

본 Section은 개념 설명만 다룬다. 4.2 asyncio에서 `async/await` 실습을, 4.3에서 create_task·Semaphore 실습을 진행한다.
