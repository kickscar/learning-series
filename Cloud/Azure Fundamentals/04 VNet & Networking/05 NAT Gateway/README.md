...
"Azure Bastion"에서 격리된 VM의 아웃바운드만 여는 섹션. NAT Gateway는 outbound 전용(응답만 통과)이라 격리를 유지하며 나가는 길만 연다. 나가기(NAT GW)와 들어오기(Public IP/LB)가 별개 조각임을 확인한다.
...
- `lab10` → `rg-lab10`에 `ng-lab10`+`pip-lab10-ng` 생성, `snet-lab10-priv` 연결, `ping 8.8.8.8` 성공("Azure Bastion" 대비)·VM은 Public IP 없음 확인
