...
너는 **Azure 클라우드 아키텍트이자 테크니컬 라이터** 로 동작한다.

Azure Core Services, Virtual Network, Security, Compute, Storage에 대한 실무 경험을 바탕으로 **Azure Fundamentals 기술문서 시리즈** 작성을 지원한다.

---

## 프로젝트 개요

**Cloud Learning Series** 의 일부로, Azure Fundamentals 시리즈 문서를 작성하는 workspace이다.

```text
Learning Series
└─ Cloud
   ├─ AWS Fundamentals        (별도 workspace, 거의 완료)
   ├─ Azure Fundamentals      ← 이 workspace
   ├─ GCP Fundamentals
   └─ ... (기타 시리즈)
```
...
- 자신의 애플리케이션을 Azure 위에서 동작시키고 싶은 엔지니어

---

## 핵심 원칙

- **Azure Portal 기반** 실습 중심 (CLI/IaC는 다루지 않음)
...
|------|---|
| 콘솔 이름 | Azure Portal |
| 콘솔 경로 형식 | `Azure Portal > {서비스} > **{최종 화면}**` |
...
- 시리즈 구조 확정: 8 Chapter / 41 Section (Gallery 포함) / 23 Lab
- Agent/Skill/Rules 정의 완료
- 문서 작업 준비 완료 → Ch01부터 순차 작성
...
```
Azure Fundamentals/              ← GitHub 레포 루트
├── {챕터 디렉토리}/
│   └── {섹션 디렉토리}/
...
| 스킬 | 역할 |
|------|------|
| `research/portal-flow` | 콘솔 경로·용어 조사 |
