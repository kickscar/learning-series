...
- **유형**: 이론 + 실습
- **내용**:
  - 변수 우선순위 체계: -var/-var-file > *.auto.tfvars > terraform.tfvars > TF_VAR_* > default
  - `terraform.tfvars` — 공통 설정 (Idempotency & Tracing)
  - `-var-file` — 환경별 분리 (`dev.tfvars`, `prod.tfvars`)
  - `TF_VAR_*` — 보안 및 CI/CD 파이프라인
  - default 전략 — sane defaults vs critical (no default)
  - sensitive 변수 처리 (plan 마스킹, State 평문 저장)
  - Sec01~03에서 다룬 환경 분리 방식과 변수 입력 전략의 조합
- **Lab**:
  - `[실습] lab01: tfvars 파일 분리와 우선순위`
    - terraform.tfvars + prod.tfvars + -var-file + -var 우선순위 실험
  - `[실습] lab02: 환경변수 & sensitive 변수`
    - TF_VAR_* 전달, sensitive 마스킹 확인
- **리서치 참고**: `.claude/research/variable-input-patterns.research.md`

---

### Section 05 — [실습] Gallery: dev/prod 환경 분리
- **유형**: 시리즈 실습
- **전제 조건**: Ch06 Gallery 완료 (모듈화 상태), Ch07 Sec02~03 완료
- **내용**:
  - Ch06 Gallery의 모듈화된 Gallery 인프라를 dev/prod로 분리
...
- **예제 파일**: `07 환경 분리/05 [실습] Gallery: dev·prod 환경 분리/`
